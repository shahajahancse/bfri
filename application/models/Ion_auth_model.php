...
class Ion_auth_model extends CI_Model
{
	...
	public function login_apps($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, active, last_login, first_name, created_on, profile_img, is_request')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  		->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, role, '.$this->tables['groups'].'.description, '.$this->tables['groups'].'.pw, '.$this->tables['groups'].'.permission' )
